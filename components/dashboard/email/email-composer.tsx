  ...
  return (
    <>
      <div className="fixed inset-x-0 bottom-0 z-40 flex justify-center sm:inset-x-auto sm:right-6 sm:justify-end">
        <div className={cn("flex w-full flex-col overflow-hidden bg-card sm:w-[512px] sm:max-w-[calc(100vw-3rem)] sm:rounded-t-xl sm:border sm:border-border sm:shadow-2xl", composeMinimized ? "h-auto" : "h-[100svh] sm:h-[560px] sm:max-h-[calc(100svh-2rem)]")}>
          {/* Full-page header on mobile (back arrow); docked popup chrome on desktop. */}
          <div className="flex shrink-0 items-center justify-between gap-2 border-b border-border bg-background px-3 py-2.5 text-foreground sm:border-0 sm:bg-neutral-800 sm:px-4 sm:py-2 sm:text-white sm:dark:bg-neutral-900">
            <button type="button" onClick={closeCompose} aria-label="Back" className="-ml-1 flex size-8 shrink-0 items-center justify-center rounded outline-none transition-colors hover:bg-muted sm:hidden"><ArrowLeft className="size-5" aria-hidden="true" /></button>
            <span className="min-w-0 flex-1 truncate text-sm font-medium sm:hidden">{subject.trim() || "New message"}</span>
            <button type="button" onClick={() => setComposeMinimized(!composeMinimized)} className="hidden min-w-0 flex-1 truncate text-left text-sm font-medium outline-none sm:block" title={composeMinimized ? "Expand" : "Minimize"}>{subject.trim() || "New message"}</button>
            <div className="hidden shrink-0 items-center gap-1 sm:flex"><button type="button" onClick={() => setComposeMinimized(!composeMinimized)} aria-label={composeMinimized ? "Expand" : "Minimize"} className="flex size-7 items-center justify-center rounded text-white/80 outline-none transition-colors hover:bg-white/15 hover:text-white"><ChevronDown className={cn("size-4 transition-transform", composeMinimized && "rotate-180")} aria-hidden="true" /></button><button type="button" onClick={closeCompose} aria-label="Close" className="flex size-7 items-center justify-center rounded text-white/80 outline-none transition-colors hover:bg-white/15 hover:text-white"><X className="size-4" aria-hidden="true" /></button></div>
          </div>
