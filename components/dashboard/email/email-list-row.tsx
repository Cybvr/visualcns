...
export function EmailListRow({
  ...
  return (
    <div
      ...
      className={cn(
        "group flex cursor-pointer items-center gap-3 border-b border-border p-2 outline-none transition-colors last:border-b-0 hover:bg-muted/50 focus-visible:bg-muted/50",
    ...
    >
      ...
      <div className={cn("flex min-w-0 flex-1 flex-col gap-0.5", !compact && "sm:contents")}>
        <div className={cn("flex min-w-0 items-center justify-between gap-2", !compact && "sm:w-[34%] sm:shrink-0 sm:justify-start")}>
          ...
          {formattedDate && date ? (
            <time dateTime={date} className={cn("shrink-0 text-muted-foreground", !compact && "sm:hidden")}>{formattedDate}</time>
          ) : null}
        </div>
        <span className="min-w-0 flex-1 truncate text-muted-foreground">{subject}</span>
      </div>
      {!compact && formattedDate && date ? (
        <time dateTime={date} className="hidden w-24 shrink-0 text-right text-muted-foreground sm:block">{formattedDate}</time>
      ) : null}
      {onDelete ? (
        <button
          type="button"
          ...
          className="hidden size-8 shrink-0 items-center justify-center rounded-sm text-muted-foreground opacity-0 transition-opacity hover:bg-destructive/10 hover:text-destructive focus-visible:opacity-100 group-hover:opacity-100 sm:inline-flex"
        >
