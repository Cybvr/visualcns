...
async function authorize(request: NextRequest) {
  ...
  return { db, agencyId: requireAgencyId(data) }
}

async function scanOrganizations(db: FirebaseFirestore.Firestore, agencyId: string) {
  const snapshot = await db.collection("organizations").where("agencyId", "==", agencyId).get()
...
async function migrateOrganizations(db: FirebaseFirestore.Firestore, agencyId: string) {
  const { snapshot, missing } = await scanOrganizations(db, agencyId)
  const used = new Set(
    snapshot.docs
...
export async function GET(request: NextRequest) {
  try {
    const { db, agencyId } = await authorize(request)
    const { snapshot, missing } = await scanOrganizations(db, agencyId)
    return NextResponse.json({ agencyId, total: missing.length, organizationCount: snapshot.size, organizations: missing })
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    const { db, agencyId } = await authorize(request)
    const { migrated, organizationCount } = await migrateOrganizations(db, agencyId)
    return NextResponse.json({ agencyId, total: migrated.length, organizationCount, organizations: migrated })
  } catch (error) {
