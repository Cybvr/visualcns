...
async function caller(request: NextRequest) {
  ...
  return { db, uid: decoded.uid, tenantId: typeof data.tenantId === "string" && data.tenantId ? data.tenantId : "legacy-visualcns" }
}

export async function POST(request: NextRequest) {
  try {
    const { db, uid, tenantId } = await caller(request)
...
export async function GET(request: NextRequest) {
  try {
    const { db, tenantId } = await caller(request)
    const snapshot = await db.collection("invites").where("tenantId", "==", tenantId).orderBy("createdAt", "desc").get()
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (data.tenantId !== tenantId || data.status !== "pending" || data.expiresAt?.toMillis?.() < Date.now()) {
