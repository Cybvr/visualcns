...
export async function POST(request: Request) {
  ...
  try {
    ...
    let tenantId = String(object.metadata?.tenantId || object.subscription_details?.metadata?.tenantId || "")
    if (!tenantId && object.customer) {
      const match = await db.collection("tenants").where("stripeCustomerId", "==", object.customer).limit(1).get()
      tenantId = match.empty ? "" : match.docs[0].id
    }
    if (tenantId) {
      ...
      await db.collection("tenants").doc(tenantId).set({ ...(status ? { status } : {}), ...(object.customer ? { stripeCustomerId: object.customer } : {}), ...(object.subscription ? { stripeSubscriptionId: object.subscription } : {}), updatedAt: FieldValue.serverTimestamp() }, { merge: true })
    }
