...
export default function AgencyDataPage() {
  ...
  return <main className="mx-auto w-full max-w-3xl px-4 py-9 sm:px-6"><AccountNav /><header className="mt-7"><h1 className="text-lg font-semibold">Workspace data</h1><p className="mt-1 text-sm leading-6 text-muted-foreground">Export or permanently remove this agency tenant’s records.</p></header><div className="mt-6 space-y-8"><section className="border border-border p-5"><h2 className="font-medium">Export</h2><p className="mt-1 text-sm text-muted-foreground">Download a JSON copy of the tenant-scoped workspace data.</p><Button className="mt-4" variant="outline" disabled={busy} onClick={() => void exportData()}>Download export</Button></section><section className="border border-destructive/40 p-5"><h2 className="font-medium">Delete workspace data</h2><p className="mt-1 text-sm text-muted-foreground">This cannot be undone. Type the confirmation phrase returned by the server.</p><Label className="mt-4 block text-sm" htmlFor="delete-confirmation">Confirmation</Label><Input id="delete-confirmation" className="mt-1 max-w-sm" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder="DELETE your-tenant-id" /><Button className="mt-4" variant="destructive" disabled={busy || !confirmation} onClick={() => void deleteData()}>Delete tenant data</Button></section>{message && <p className="text-sm text-muted-foreground">{message}</p>}</div></main>
}
